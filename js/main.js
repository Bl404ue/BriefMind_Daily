...
function fetchPapers(date = selectedDate) {
    ...
    fetch(filename)
        ...
        .then(csvText => {
            ...
            // 解析 CSV 文本
            const papers = parseHuggingFaceCSV(csvText);
            console.log('Processed HuggingFace papers:', papers.length);
        ...
        .catch(error => {
            ...
            preloadedData.papers = []; // 设置为空数组而不是null
            
            // 检查是否已经有arxiv论文数据
            if (preloadedData.arxivPapers) {
                // 如果有arxiv论文数据，则只渲染arxiv论文
                renderArxivPapers();
            } else {
                // 如果两种论文数据都没有，才显示错误信息
                document.getElementById('huggingface-papers-container').innerHTML = 
                    '<div class="col-12 text-center mt-4 mb-4"><div class="empty-state"><i class="fas fa-file-alt fa-3x mb-3"></i><h4>暂无论文</h4></div></div>';
            }
...
function fetchArxivPapers(date = selectedDate) {
    ...
    fetch(filename)
        ...
        .then(csvText => {
            ...
            // 解析 CSV 文本
            const papers = parseArxivCSV(csvText);
            console.log('Processed Arxiv papers:', papers.length);
        ...
        .catch(error => {
            ...
            preloadedData.arxivPapers = []; // 设置为空数组而不是null
            
            // 检查是否已经有huggingface论文数据
            if (preloadedData.papers) {
                // 如果有huggingface论文数据，则只渲染huggingface论文
                renderPapers();
            } else if (document.getElementById('huggingface-papers-container').innerHTML.indexOf('spinner-border') !== -1) {
                // 如果两种论文数据都没有，且容器中只有加载动画，才显示错误信息
                document.getElementById('huggingface-papers-container').innerHTML = 
                    '<div class="col-12 text-center mt-4 mb-4"><div class="empty-state"><i class="fas fa-file-alt fa-3x mb-3"></i><h4>暂无论文</h4></div></div>';
            }
...
function renderWechatArticles(articles) {
    ...
    if (!articles || articles.length === 0) {
        ...
        if (isFiltering && preloadedData.wechatArticles && preloadedData.wechatArticles.length > 0) {
            // 筛选导致的空结果
            container.innerHTML = '<div class="col-12 text-center mt-4 mb-4"><div class="empty-state"><i class="fas fa-filter fa-3x mb-3"></i><h4>没有符合要求的推文</h4></div></div>';
        } else {
            // 日期筛选或数据本身为空
            container.innerHTML = '<div class="col-12 text-center mt-4 mb-4"><div class="empty-state"><i class="fas fa-inbox fa-3x mb-3"></i><h4>暂无推文</h4></div></div>';
        }
        return;
    }
...
function renderCombinedPapers(filteredPapers, filteredArxivPapers) {
    ...
    combinedPapers.forEach(paper => {
        ...
        if (originalTitleElement) {
            if (paper['中文标题']) {
                // 处理中文标题中可能存在的换行符
                originalTitleElement.textContent = paper['中文标题'].replace(/\n/g, ' ').trim();
            } else {
        ...
        if (summaryTextElement && paper['简明摘要']) {
            ...
            // 处理简明摘要中可能存在的换行符
            const cleanedSummary = paper['简明摘要'].replace(/\n/g, ' ').trim();
        ...
        if (summaryContentElement) {
            // 处理简明摘要中可能存在的换行符
            summaryContentElement.textContent = paper['简明摘要'] ? paper['简明摘要'].replace(/\n/g, ' ').trim() : '无摘要';
        }
...
function resetFilters() {
    ...
    preloadedData.selectedFields.add('all');
...
function populateFilterOptions() {
    ...
    if (institutionOptions) {
        ...
        PREDEFINED_INSTITUTIONS.forEach(institution => {
            ...
            const checkbox = document.createElement('input');
            checkbox.type = 'checkbox';
            checkbox.id = `institution-${institution.replace(/\s+/g, '-').toLowerCase()}`;
